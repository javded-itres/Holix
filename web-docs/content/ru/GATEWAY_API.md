# Holix Gateway API — полный справочник
...
Holix запускает **единый multi-profile HTTP gateway** с тремя публичными поверхностями:
...
| **Расширения агента Holix** | `/v1/chat/completions`, permissions, plans | OpenAI chat + разрешения на инструменты + ревью планов |
| **Holix Management API** | `/api/holix/` | SaaS control plane: профили, модели, MCP, навыки, Telegram |
...
| `/api/sessions` CRUD + chat/stream | Полная | `source`, `include_children`; персистентность в `~/.holix/data/gateway/sessions.json` |
...
| Только Holix | Дополнительно | `/api/holix/*`, permissions, plans — нет в Hermes |
...
3. В поле **HolixApiKey** вставьте ключ gateway (`hx_…`) **без** префикса `Bearer` — Swagger добавит его автоматически
...
В зависимости от маршрута Holix использует до **трёх независимых механизмов аутентификации**.

### Уровень 1 — API key gateway (`hx_…`)

Требуется почти на всех маршрутах при `HOLIX_REQUIRE_AUTH=true` (по умолчанию).
...
```bash
# Нужен существующий admin key ИЛИ bootstrap с HOLIX_REQUIRE_AUTH=false один раз
...
# Сохраните hx_…, установите HOLIX_REQUIRE_AUTH=true, holix gateway reload
```

### Уровень 2 — Ключ доступа к профилю (`hp_…`)

Требуется для маршрутов `/api/holix/*` (дополнительно к API key gateway):
...
|--------------------|-----------|---------|
| Владелец профиля | Gateway key + `X-Holix-Profile-Key` для своего профиля | Один профиль |
| Админ платформы | Gateway key с правом `admin` **или** master key admin-профиля | Все профили |

Имя admin-профиля по умолчанию: `admin` (`HOLIX_TELEGRAM_ADMIN_PROFILE`).

Ключи профиля создаются через CLI (`holix profile key init`) или Management API (`POST …/key/init`). Это **не** то же самое, что gateway keys `hx_…`.

### Уровень 3 — Токен docs-chat (только для виджета сайта)

Маршруты под `/v1/docs/chat/*` (кроме `/config`) используют `HOLIX_DOCS_CHAT_TOKEN`:
...
1. Заголовок `X-Holix-Profile` или `X-Hermes-Profile`
...
|------------|-------|--------------|
| Id разговора / транскрипта | `X-Holix-Session-Id` | `X-Hermes-Session-Id` |
| Стабильная область памяти | `X-Holix-Session-Key` | `X-Hermes-Session-Key` |
...
У каждого API key есть `rate_limit` (запросов в минуту). Docs-chat имеет отдельный `HOLIX_DOCS_CHAT_RATE_LIMIT_RPM` на `client_id`.
...
Список профилей Holix в формате OpenAI models (`id` = имя профиля).
...
| Поле | Тип | Описание |
|------|-----|----------|
| `model` | string | Имя профиля (по умолчанию `holix`) |
...
Эндпоинты агента, специфичные для Holix (OpenAI chat, permissions, plans).
...
Список сессий. Query: `limit` (50), `offset` (0). Профиль через `X-Holix-Profile`.
...
Префикс `/api/holix/profiles`. API key gateway + доступ к профилю (см. [Аутентификация](#аутентификация)).
...
Префикс `/api/holix/profiles/{profile_id}/models`.
...
Префикс `/api/holix/profiles/{profile_id}/skills`.
...
Префикс `/api/holix/profiles/{profile_id}/mcp`.
...
Префикс `/api/holix/profiles/{profile_id}`.
...
Префикс `/api/holix/global`. Требуется доступ к **Admin**-профилю.

### `POST /api/holix/global/init`

Создать шаблоны `~/.holix/global/config.yaml` и `.env`.
...
Префикс `/api/holix/profiles/{profile_id}/telegram`. CLI-эквиваленты — в [TELEGRAM.md](TELEGRAM.md).
...
Telegram admin user id и сопоставленный профиль Holix.
...
Соответствие user id → профиль Holix.
...
Rate limit на `client_id` (`HOLIX_DOCS_CHAT_RATE_LIMIT_RPM`).
...
- Host profile задаётся при старте (`HOLIX_PROFILE`)
...
- Установите `HOLIX_API_KEY_PEPPER` в production (обязательно при `HOLIX_ENV=production`)
- Admin-маршруты всегда требуют право admin
- Security headers: `X-Content-Type-Options: nosniff`, `Referrer-Policy: no-referrer`
- CORS: `HOLIX_CORS_ORIGINS` (через запятую)
