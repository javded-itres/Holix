"""``holix max admin`` — view or clear the single MAX administrator."""

from __future__ import annotations

from integrations.max.admin import (
    clear_admin_user,
    load_admin_holix_profile,
    load_admin_user_id,
)
from integrations.max.env_store import load_max_env_files

from cli.utils.rich_console import print_error, print_info, print_success, print_warning


def max_admin_show(bot_profile: str) -> None:
    load_max_env_files(bot_profile)
    admin_id = load_admin_user_id(bot_profile)
    if admin_id is None:
        print_warning("MAX-администратор не назначен.")
        print_info(
            f"Назначить при первом approve: "
            f"holix -p {bot_profile} max requests approve USER_ID --set-admin"
        )
        return

    holix_profile = load_admin_holix_profile(bot_profile)
    print_success(f"MAX admin: user id {admin_id}")
    print_info(f"Профиль Holix: {holix_profile}")
    print_info("Назначение и смена — только через CLI (не из MAX).")


def max_admin_clear(bot_profile: str) -> None:
    load_max_env_files(bot_profile)
    if load_admin_user_id(bot_profile) is None:
        print_info("Администратор не был назначен.")
        return
    if not clear_admin_user(bot_profile):
        print_error("Не удалось сбросить администратора.")
        raise SystemExit(1)
    print_success("MAX-администратор сброшен.")
    print_info(
        f"Назначить снова: holix -p {bot_profile} max requests approve USER_ID --set-admin"
    )