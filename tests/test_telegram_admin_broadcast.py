"""Admin broadcast to Telegram users."""
...
from integrations.telegram.admin_broadcast import (
    AdminBroadcastDraft,
...
def _setup_bot(holix_home) -> None:
    save_telegram_env({"TELEGRAM_BOT_TOKEN": "123456789:AAHtesttoken"}, profile="shared")
...
async def test_compose_sends_and_clears_draft(holix_home, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_deliver(bot_profile, recipients, content, *, target, bot=None):
        delivered.extend(uid for uid, _ in recipients)
        from integrations.telegram.admin_broadcast import BroadcastDeliveryResult

        return BroadcastDeliveryResult(sent=len(recipients))

    monkeypatch.setattr(
        "integrations.telegram.admin_broadcast.deliver_broadcast",
        _fake_deliver,
    )
