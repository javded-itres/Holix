...
def test_create_profile_open_by_default(helix_home: Path) -> None:
...
def test_create_profile_with_access_key(helix_home: Path) -> None:
    manager = ProfileManager()
...
def test_env_profile_key_unlocks(helix_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = ProfileManager()
    ...
    init_profile("default", prompt_key=False)
    monkeypatch.setenv("HELIX_PROFILE_KEY", access_key)
    config = init_profile("carol", prompt_key=False)
    assert config.profile_name == "carol"


def test_default_profile_switch_without_key(helix_home: Path) -> None:
    manager = ProfileManager()
...
def test_load_profile_unknown_raises(helix_home: Path) -> None:
    manager = ProfileManager()
    with pytest.raises(ProfileNotFoundError):
        manager.load_profile("missing")


def test_key_init_and_verify(helix_home: Path) -> None:
    manager = ProfileManager()
...
def test_disable_access_key_allows_free_switch(helix_home: Path) -> None:
    manager = ProfileManager()
