"""Helix centralized logging and ``helix logs`` command."""
...
from core.logging.setup import configure_helix_logging, set_debug_enabled
from core.logging.state import LoggingState, load_logging_state, save_logging_state
from typer.testing import CliRunner


@pytest.fixture
def helix_logs_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    logs = tmp_path / "logs"
    ...
    monkeypatch.setattr("core.logging.paths.HELIX_HOME", tmp_path)
...
def test_logging_state_roundtrip(helix_logs_home: Path) -> None:
...
def test_append_agent_event_jsonl(helix_logs_home: Path) -> None:
...
def test_parse_json_and_plain_lines(tmp_path: Path) -> None:
    ...
    plain_path = tmp_path / "helix.log"
    plain_path.write_text("2026-06-06 12:00:01 INFO     [helix] started\n", encoding="utf-8")
...
def test_read_log_entries_filter_level(helix_logs_home: Path) -> None:
...
def test_rotate_file_when_over_limit(helix_logs_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    path = helix_logs_home / "logs" / "helix.log"
...
def test_debug_toggle(helix_logs_home: Path) -> None:
    set_debug_enabled(True)
    assert load_logging_state().debug_enabled is True
    log_subagent_event("DEBUG", "heartbeat", subagent="worker-1")
    debug_path = helix_logs_home / "logs" / "agent.debug.jsonl"
    assert debug_path.exists()


def test_discover_log_files(helix_logs_home: Path) -> None:
...
def test_cli_logs_list(helix_logs_home: Path) -> None:
    runner = CliRunner()
    result = runner.invoke(app, ["logs", "list"])
    assert result.exit_code == 0
    assert "Helix logs" in result.stdout


def test_cli_logs_show(helix_logs_home: Path) -> None:
...
def test_cli_logs_debug_status(helix_logs_home: Path) -> None:
    runner = CliRunner()
...
def test_configure_helix_logging_idempotent(helix_logs_home: Path) -> None:
    configure_helix_logging(force=True)
    configure_helix_logging()
    assert (helix_logs_home / "logs" / "helix.log").parent.exists()