...
## Notes & gotchas
- …""",
        "init.user_message": """Run a **deep project onboarding** for the current working directory.

## Goal
Produce an exhaustive, accurate project handbook at **`{path}`** (overwrite or create).
...
## Output
- Write the final document with `write_file` to **`{path}`**.
...
- …""",
        "init.user_message": """Выполни **глубокую инициализацию проекта** для текущей рабочей директории.

## Цель
Подготовь исчерпывающий и точный справочник проекта в **`{path}`** (перезаписать или создать).
Все будущие агенты в этом репозитории будут опираться на этот файл в первую очередь.

## Как работать
1. Активно используй tools: `list_directory` (рекурсивно при необходимости), `read_file`, `run_terminal_command` для безопасного обзора (`find`, `tree`, `rg`, манифесты пакетов).
2. Прочитай **всю** релевантную документацию: README*, CONTRIBUTING*, docs/, OpenAPI/Swagger, ADR, `.env.example`, CI-конфиги.
3. Опиши **структуру папок** с краткими пояснениями.
4. Выяви **архитектуру**: слои, DI, потоки событий, основные бинарники/сервисы.
5. Задокументируй **REST/HTTP** (или GraphQL/gRPC): маршруты, контроллеры/handlers, middleware, auth, DTO.
6. Перечисли **ключевые модули**, публичные API и зависимости между пакетами.
7. Зафиксируй **запуск, тесты, сборку, деплой** по реальным конфигам (без догадок).
8. Кратко опиши **стиль кода** (линтеры, форматтеры, паттерны в коде).
9. Если что-то неясно — изучи код до уверенности; неизвестное явно отметь в документе.

## Результат
- Запиши итоговый документ через `write_file` в **`{path}`**.
- Используй структуру ниже (заполни каждый раздел; добавляй подразделы по необходимости).
- Будь **конкретным** (пути к файлам, имена модулей, примеры команд).
