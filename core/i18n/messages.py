...
MESSAGES: dict[str, dict[str, str]] = {
    "en": {
        ...
        "models_hint": "Models: holix models refresh <provider> | holix models default <provider> <model>",
        ...
        "tg.code_mode.profile": "Code mode (main): {mode}",
        "tg.code_mode.set": "Code mode → {mode}",
        "tg.subagent_types.hint": "Tap a type to edit. Built-in types can be customized (personality, model, temperature, tools, Code mode).",
        "tg.subagent_types.empty": "No sub-agent types.",
        "tg.subagent_types.builtin": "built-in",
        "tg.subagent_types.custom": "custom",
        "tg.subagent_types.create": "+ Create from description",
        "tg.subagent_types.create_prompt": "Send a short role description (one message). /cancel to abort.",
        "tg.subagent_types.created": "Created {name} — it is in the list and ready to spawn.",
        "tg.subagent_types.create_fail": "Could not create type: {error}",
        "tg.subagent_types.working": "Working…",
        "tg.subagent_types.personality": "Personality",
        "tg.subagent_types.gen_personality": "Generate personality",
        "tg.subagent_types.write_personality": "Write personality",
        "tg.subagent_types.personality_prompt": "Send the new personality text. /cancel to abort.",
        "tg.subagent_types.personality_done": "Personality updated: {name}",
        "tg.subagent_types.write_desc": "Edit description",
        "tg.subagent_types.desc_prompt": "Send a short description (one message). /cancel to abort.",
        "tg.subagent_types.desc_done": "Description updated: {name}",
        "tg.subagent_types.model": "Model",
        "tg.subagent_types.temp": "Temperature",
        "tg.subagent_types.presentation": "Code mode",
        "tg.subagent_types.tools": "Tools",
        "tg.subagent_types.tools_hint": "Toggle tools for this type. Built-in types keep the rest of the system defaults.",
        "tg.subagent_types.tools_min": "Keep at least one tool.",
        "tg.subagent_types.tool_toggled": "{tool}: {state}",
        "tg.subagent_types.inherit_model": "inherit parent",
        "tg.subagent_types.temp_set": "{name}: temperature {value}",
        "tg.subagent_types.presentation_set": "{name}: Code mode → {mode}",
        "tg.subagent_types.model_set": "{name}: model updated",
        "tg.subagent_types.delete": "Delete",
        "tg.subagent_types.delete_confirm": "Confirm delete",
        "tg.subagent_types.delete_hint": "Tap Confirm delete to remove {name}.",
        "tg.subagent_types.deleted": "Deleted {name}",
        "tg.subagent_types.reset": "Reset to default",
        "tg.subagent_types.reset_done": "Reset {name} to built-in defaults",
        "tg.subagent_types.back": "‹ Back",
        ...
        "tg.cmd.todos": "Session checklist",
        "tg.cmd.trace": "Session trajectory",
        "tg.cmd.permission": "Sandbox / approval preset",
        "tg.cmd.pty": "Persistent shell",
        "tg.cmd.change": "SDD git worktree",
        ...
        "tui.help.keys5": "  Enter while the agent is busy queues the prompt (click to edit, × to drop)",
        "tui.queue.title": "Queue ({n})",
        "tui.queue.hint": "click to edit · × delete",
        "tui.queue.edit": "edit",
        "tui.queue.full": "Queue is full (50). Delete an item or wait.",
        ...
        "tui.subagent_types.list_hint": "Built-in types: Edit overrides, Delete resets · Create custom types with prompt, skills, MCP, model, CLI",
        "tui.subagent_types.reset": "Reset",
        "tui.subagent_types.reset_done": "Built-in type reset to code defaults: {name}",
        ...
        "tui.subagent_types.select_type": "Select a type",
...
When finished, confirm the absolute path written and give a 5–10 line summary of what you captured.""",
    ...
    "ru": {
        ...
        "models_hint": "Модели: holix models refresh <провайдер> | holix models default <провайдер> <модель>",
        ...
        "tg.code_mode.profile": "Code mode (main): {mode}",
        "tg.code_mode.set": "Code mode → {mode}",
        "tg.subagent_types.hint": "Нажмите тип для настройки. Встроенные тоже можно менять (личность, модель, температура, tools, Code mode).",
        "tg.subagent_types.empty": "Нет типов субагентов.",
        "tg.subagent_types.builtin": "системный",
        "tg.subagent_types.custom": "свой",
        "tg.subagent_types.create": "+ Создать по описанию",
        "tg.subagent_types.create_prompt": "Пришлите короткое описание роли одним сообщением. /cancel — отмена.",
        "tg.subagent_types.created": "Создан {name} — он сразу в списке и доступен для запуска.",
        "tg.subagent_types.create_fail": "Не удалось создать тип: {error}",
        "tg.subagent_types.working": "Готовлю…",
        "tg.subagent_types.personality": "Личность",
        "tg.subagent_types.gen_personality": "Сгенерировать личность",
        "tg.subagent_types.write_personality": "Написать личность",
        "tg.subagent_types.personality_prompt": "Пришлите текст личности. /cancel — отмена.",
        "tg.subagent_types.personality_done": "Личность обновлена: {name}",
        "tg.subagent_types.write_desc": "Описание",
        "tg.subagent_types.desc_prompt": "Пришлите короткое описание одним сообщением. /cancel — отмена.",
        "tg.subagent_types.desc_done": "Описание обновлено: {name}",
        "tg.subagent_types.model": "Модель",
        "tg.subagent_types.temp": "Температура",
        "tg.subagent_types.presentation": "Code mode",
        "tg.subagent_types.tools": "Tools",
        "tg.subagent_types.tools_hint": "Включайте и выключайте tools для этого типа. У системных остальное поведение как в Holix.",
        "tg.subagent_types.tools_min": "Нужен хотя бы один tool.",
        "tg.subagent_types.tool_toggled": "{tool}: {state}",
        "tg.subagent_types.inherit_model": "как у main",
        "tg.subagent_types.temp_set": "{name}: температура {value}",
        "tg.subagent_types.presentation_set": "{name}: Code mode → {mode}",
        "tg.subagent_types.model_set": "{name}: модель обновлена",
        "tg.subagent_types.delete": "Удалить",
        "tg.subagent_types.delete_confirm": "Точно удалить",
        "tg.subagent_types.delete_hint": "Нажмите «Точно удалить», чтобы убрать {name}.",
        "tg.subagent_types.deleted": "Удалён {name}",
        "tg.subagent_types.reset": "Сбросить",
        "tg.subagent_types.reset_done": "{name} сброшен к системному",
        "tg.subagent_types.back": "‹ Назад",
        ...
        "tg.cmd.todos": "Чеклист сессии",
        "tg.cmd.trace": "Траектория сессии",
        "tg.cmd.permission": "Пресет песочницы / подтверждений",
        "tg.cmd.pty": "Постоянный shell",
        "tg.cmd.change": "SDD git worktree",
        ...
        "tui.help.keys5": "  Enter пока агент занят ставит промпт в очередь (клик — править, × — убрать)",
        "tui.queue.title": "Очередь ({n})",
        "tui.queue.hint": "клик — править · × удалить",
        "tui.queue.edit": "править",
        "tui.queue.full": "Очередь заполнена (50). Удалите пункт или дождитесь хода.",
        ...
        "tui.subagent_types.list_hint": "Встроенные типы: Изменить переопределяет, Удалить сбрасывает · Свои типы: промпт, skills, MCP, модель, CLI",
        "tui.subagent_types.reset": "Сбросить",
        "tui.subagent_types.reset_done": "Встроенный тип сброшен к настройкам из кода: {name}",
        ...
        "tui.subagent_types.select_type": "Выберите тип",
