"""Profile management, storage paths, and session bootstrap (core layer)."""
...
def init_profile(
    ...
    *,
    ...
    if created_key:
        # Surface once for operators; CLI may wrap with Rich separately.
        import logging
        import sys

        msg = (
            f"Profile access key for '{profile}' (shown once): {created_key}\n"
            f"Use: holix -p {profile} --profile-key <key>"
        )
