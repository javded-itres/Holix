...
Helix выполняет shell-команды через инструмент `run_terminal_command`. До запуска в ОС команда проходит **несколько независимых проверок**.
...
flowchart TD
  ...
  B -->|HELIX_ENABLE_TERMINAL_TOOL=false| X1[Отказ: инструмент выключен]
  B -->|да| C{Подтверждение ActionGuard}
  C -->|пользователь отказал| X2[Отказ: denied by user]
  C -->|одобрено / auto-allow| D{Whitelist включён?}
  D -->|HELIX_TERMINAL_COMMAND_WHITELIST=false| F[Запуск в shell]
...
|------------|--------------|--------|
| `HELIX_ENABLE_TERMINAL_TOOL` | `true` | Главный переключатель `run_terminal_command` |
...
| Переменная | По умолчанию | Эффект |
|------------|--------------|--------|
| `HELIX_TERMINAL_COMMAND_WHITELIST` | `true` | Включить allowlist |
| `HELIX_TERMINAL_WHITELIST_EXTRA` | пусто | Доп. команды/префиксы через запятую |
...
| `helix gateway status` | Разрешено | префикс `helix` |
...
`dir`, `type`, `more`, `findstr`, `where`, `cd`, `echo`, `tree`, `whoami`, `hostname`, `date`, `systeminfo`, `tasklist`, `ipconfig`, `ping`, `curl`, `nslookup`, а также git/python/npm/pytest/helix/uv (`py` на Windows).

Команды Unix (`ls`, `grep`) в списке Windows **отсутствуют** — используйте `dir` / `findstr` или добавьте в extras.

### Если whitelist отключён

`HELIX_TERMINAL_COMMAND_WHITELIST=false` отключает allowlist (опасные паттерны остаются). Только для полностью доверенной dev-среды.
...
| Docker / make / свой CLI | `helix profile whitelist add "docker, make"` |
...
1. Оставляйте `HELIX_TERMINAL_COMMAND_WHITELIST=true`.
2. Добавляйте в extras только **минимум** (`whitelist add`).
3. Используйте **workspace jail** на общих хостах.
4. Отключите терминал, если не нужен: `HELIX_ENABLE_TERMINAL_TOOL=false`.
5. Отключите Python executor: `HELIX_ENABLE_CODE_EXECUTOR=false`.
6. Запускайте `helix doctor`, см. [SECURITY.md](SECURITY.md).
