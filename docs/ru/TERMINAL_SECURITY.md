...
|--------|--------|------------|
| `workspace-write` | только workspace + tmp | macOS `sandbox-exec`, Linux `bwrap` |
| `read-only` | нет | мутирующие инструменты (`write_file`, `patch_file`, …) отклоняются |
| `danger-full-access` | без обёртки | по умолчанию, если jail выключен |

Если задан ограниченный пресет, а бэкенда нет, команда **отклоняется**. См. [SECURITY.md](SECURITY.md).
