...
`/help` (и `/start`) открывают **меню сценариев** с кнопками: начало, чат, субагенты, навыки, SDD, модели, память, cron, MCP, права, файлы, список команд.

У субагентов своё подменю: что это, **настройка типов** (`/subagent-types`, создание по описанию, оверлеи), запуск, Code mode. Коротко: `/help субагенты`.

Панель настроек по-прежнему `/menu` (в том числе **Шаги** / `max_steps` профиля).
...
Просьба «пришли файл / md» должна уходить **вложением** через `send_chat_files`, а не текстом в пузыре. Контракт tool: [TOOLS.md](TOOLS.md).

Если бот пишет, что отправил файл, а документа в Telegram нет — напишите **«проверь себя»**. Запустится субагент `session_doctor` с чистым контекстом. Если без оператора не обойтись, доктор предложит тикет администратору Telegram — нужно подтвердить; отказ ничего не отправляет.

Входящие фото, документы и голос — ниже; этот раздел только про **исходящие** вложения.
