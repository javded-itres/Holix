"""Admin broadcast posts to Telegram users (shared bot)."""
...
from integrations.telegram.markdown import escape_html, split_telegram_html
...
def resolve_broadcast_recipients(
    ...
    *,
    ...
    """Return (telegram_user_id, holix_profile) pairs for a broadcast target."""
...
async def deliver_broadcast(
    ...
    *,
    ...
    """Send broadcast HTML to each recipient."""
            ...
            try:
                for chunk in split_telegram_html(html):
                    if aiogram_bot is not None:
                        ...
                        await send_user_message(token, int(user_id), chunk)
                    await asyncio.sleep(0.05)
                result.sent += 1
            except Exception as exc:
                result.failed.append(f"{user_id}: {exc}")
    finally:
        if close_bot and aiogram_bot is not None:
...
async def try_compose_admin_broadcast(host: Any, text: str) -> bool:
    ...
    if not message:
        await host._send_plain("Пустое сообщение не отправлено.")
        return True
